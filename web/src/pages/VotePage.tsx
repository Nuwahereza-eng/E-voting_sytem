...
import { Loader2, IdCard, Wallet } from "lucide-react";
...
export function VotePage() {
  ...
  return (
    <>
      ...
      <Card className="mb-4">
        <CardHeader>
          <CardTitle className="text-base">How will you sign?</CardTitle>
          <CardDescription>
            Everyday voters use ID + code. Wallet mode is for organisers who registered a Stellar
            public key with the community.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {mode === "id" ? (
        <IdVote defaultElectionId={initialElectionId} />
      ) : (
...
function IdVote({ defaultElectionId }: { defaultElectionId: number | null }) {
  const [step, setStep] = useState<Step>("id");

  const [voterRef, setVoterRef] = useState("");
  const [otpBusy, setOtpBusy] = useState(false);
  const [otpErr, setOtpErr] = useState<string | null>(null);
  const [session, setSession] = useState<IdSessionState | null>(null);
      ...
      {step === "code" && session && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              A 6-digit code was sent to {session.sentTo.join(", ")}. Valid until{" "}
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                disabled={electionsBusy || code.length < 4}
              >
          ...
          <CardContent className="space-y-3">
            {elections.length === 0 && (
              <p className="text-sm text-muted-foreground">
                Ask your organiser to open an election, or check back later.
              </p>
            )}
            {elections.map((e) => {
        ...
        return (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your vote is now on-chain and cannot be changed.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              {resultsVisible ? (
                <div className="text-sm text-muted-foreground">
                  Total votes in this election: {castResult.totalVotes}.
                </div>
              ) : (
              ...
              <div className="flex justify-end">
                <Button onClick={resetAll}>Done</Button>
              </div>
            </CardContent>
          </Card>
        );
        ...
        return (
          <div key={i} className="tally-row">
            <div className="tally-label">
              {o.symbol && <span style={{ marginRight: 6 }}>{o.symbol}</span>}
              {o.label}
            </div>
